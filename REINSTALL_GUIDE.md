...
如果你想了解每一步的细节，或者快速脚本失败，可以按照以下步骤手动安装。
...
## 7. 初始化 content-hub 目录结构

### 方案 A：自动创建（推荐）

```bash
# 运行 ensure_dirs 自动创建所有必需目录
python3 ~/.openclaw/skills/note-autopilot/scripts/content_pipeline.py ensure_dirs
```

这会创建完整的目录结构：

```
~/.openclaw/content-hub/
├── 00-收件箱/
├── 01-灵感与素材库/
│   ├── 1-日常灵感剪报/
│   └── 2-爆款素材片段/
...
如果你有旧的 content-hub 备份：

```bash
# 复制整个 content-hub 目录
cp -r /path/to/backup/content-hub ~/.openclaw/

# 或者只恢复关键数据
cp -r /path/to/backup/content-hub/02-选题池 ~/.openclaw/content-hub/
cp -r /path/to/backup/content-hub/05-已发布归档 ~/.openclaw/content-hub/
```
...
# 1. 验证目录结构
python3 ~/.openclaw/skills/note-autopilot/scripts/content_pipeline.py ensure_dirs
ls -la ~/.openclaw/content-hub/

# 2. 测试热点扫描
bash ~/.openclaw/skills/note-autopilot/scripts/topic_scan.sh

# 3. 检查输出文件
ls -lh ~/.openclaw/workspace-studio/output/content-pipeline/topics/
ls -lh ~/.openclaw/content-hub/02-选题池/待写选题库/
...
6. ✅ **content-hub 目录结构**（运行 `ensure_dirs` 自动创建）
...
5. **运行 `ensure_dirs` 创建 content-hub 目录结构**
6. 重启 OpenClaw
